...
import { useAdminProducts, useFlagProduct, useUnflagProduct, useDeactivateProduct } from "@/features/admin/hooks/useAdminProducts";
...
export default function AdminProductsPage() {
  ...
  const { mutate: unflag } = useUnflagProduct();
  ...
  function handleFlag(id: number) {
    flag({ id, reason }, {
      onSuccess: () => { toast.success("Product flagged."); setReason(""); },
      onError: () => toast.error("Could not flag product."),
    });
  }
    ...
    <div className="mx-auto max-w-4xl space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <Select value={flagged || "none"} onValueChange={(v) => { setFlagged(v === "none" ? "" : v); setPage(1); }}>
          ...
          <SelectContent>
            {FLAG_OPTIONS.map((o) => <SelectItem key={o.value || "none"} value={o.value || "none"}>{o.label}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
            ...
            {data?.products.map((product) => (
              <div key={product.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card p-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{product.vendor.shop_name} · ${product.price}</p>
                  {product.is_flagged && product.flagged_reason && (
                    <p className="mt-1 text-xs text-destructive">Flagged: {product.flagged_reason}</p>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {product.is_flagged ? (
                    <Button size="sm" variant="outline" onClick={() => handleUnflag(product.id)}>Unflag</Button>
                  ) : (
                    <AlertDialog onOpenChange={(open) => !open && setReason("")}>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction disabled={isFlagging || !reason} onClick={() => handleFlag(product.id)}>Flag</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  )}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>This removes it from the public catalog immediately.</AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Cancel</AlertDialogCancel>
                          <AlertDialogAction onClick={() => handleDeactivate(product.id)}>Deactivate</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  )}
